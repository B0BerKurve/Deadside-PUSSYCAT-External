	...
	while (true)
	{
		...
		if (isAimbotActive)
		{
			...
			for (int index = 0; index < EntityList_Copy.size(); ++index)
			{
				...
				auto isDead = read<char>(Entity.actor_pawn + 0x4c0); //bIsDying : 1

				auto TeamId = read<char>(Entity.actor_pawn + 0xb90);
...
auto GameCache()->VOID
{
	while (true)
	{
		...
		for (int index = 0; index < GameVars.actor_count; ++index)
		{
			...
			if (name.find(xorstr("ProcessVolume")) != std::string::npos
				|| name.find(xorstr("FastTravel")) != std::string::npos)
				continue;
			...
			if (name.find(xorstr("Character_man_01_C")) != std::string::npos || name.find(xorstr("MOB_")) != std::string::npos)
			{
...
auto RenderVisual() -> VOID
{
	...
	for (int index = 0; index < EntityList_Copy.size(); ++index)
	{
		...
		//auto PlayerName = read<FString>(Entity.actor_pawn + GameOffset.offset_player_name);
		//auto PlayerNames = read<FString>(Entity.actor_state + GameOffset.offset_player_name);

		auto Health = read<float>(Entity.actor_pawn + 0x4e0); //ASFPSBaseCharacter	MetabolismParams
		...
		if (CFG.b_Visual)
		{
			if (entity_distance < CFG.max_distance)
			{
				...
				{
					DrawOutlinedText(Verdana, xorstr("HP: ") + std::to_string(Health), ImVec2(TopBox.x, TopBox.y - 20), 16.0f, barColor, true);
					DrawOutlinedText(Verdana, xorstr("TEAMID: ") + std::to_string(TeamId), ImVec2(TopBox.x, TopBox.y - 40), 16.0f, ImColor(255, 255, 255), true);
				}
				if (CFG.b_EspHealth)
				{
				...
				if (CFG.b_EspName)
				{
				//	DrawOutlinedText(Verdana, PlayerName.ToString(), ImVec2(TopBox.x, TopBox.y), CFG.enemyfont_size, ImColor(255, 255, 255), true);
				//	DrawOutlinedText(Verdana, PlayerNames.ToString(), ImVec2(TopBox.x, TopBox.y - 20), CFG.enemyfont_size, ImColor(255, 255, 255), true);

					if (Entity.actor_name.find(xorstr("Character")) != std::string::npos)
					{
						DrawOutlinedText(Verdana, "Player", ImVec2(BottomBox.x, BottomBox.y), CFG.enemyfont_size, ImColor(255, 128, 128), true);
					}
					else if (Entity.actor_name.find(xorstr("MOB_")) != std::string::npos)
					{
						DrawOutlinedText(Verdana, "BOT", ImVec2(BottomBox.x, BottomBox.y), CFG.enemyfont_size, ImColor(128, 255, 128), true);
					}
				}

				if (CFG.b_EspDistance)
				{
					DrawOutlinedText(Verdana, xorstr("Distance: [") + std::to_string(dist) + xorstr("]"), ImVec2(BottomBox.x, BottomBox.y + 10), CFG.enemyfont_size, ImColor(255, 255, 255), true);
				}
				if (CFG.crosshair)
				{
				...
				if (CFG.allitems)
				{
					...
					//	DrawOutlinedText(Verdana, std::to_string(a), ImVec2(Screen.x, Screen.y), 14.0f, ImColor(255, 255, 255), true);
	...
	if (CFG.b_MenuShow)
	{
		...
		if (CFG.tab_index == 1)
		{
			...
			if (CFG.b_Visual)
			{
				...
				ImGui::Checkbox(xorstr("Enemy Tag"), &CFG.b_EspName);
				ImGui::Checkbox(xorstr("Distance"), &CFG.b_EspDistance);
			...
			ImGui::SliderInt(xorstr("   "), &CFG.max_distance, 1, 600);
...
int main(int argCount, char** argVector)
{
	...
	/*if (hWnd = FindWindow(NULL, (xorstr("RaidGame"))))
	{
		...
		{
			std::cout << xorstr("[-] Close the Deadside and load Driver") << std::endl;
			Sleep(3600);
			sosok2();
			exit(-1);
		}
	...
	std::cout << xorstr("[+] Press F2 in Deadside...\n\n");
	while (true)
	{
